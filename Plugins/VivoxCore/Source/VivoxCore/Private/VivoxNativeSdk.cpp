...
AccountId  VivoxNativeSdk::GetRandomAccountId(const FString &issuer, const FString &domain, const FString &prefix, const FString& display, const FString& environmentId)
{
    char *tmp = vx_get_random_user_id(TCHAR_TO_UTF8(*prefix));
    // strip off the trailing dot
    tmp[strlen(tmp) - 1] = 0;
    // strip off the leading dot
    AccountId id(issuer, tmp+1, domain, display, TOptional<TArray<FString>>(), environmentId);
    vx_free(tmp);
    return id;
}
